...
def tee_lünktekstiks(tekst):
    ...
    for lause in tekst:
        ...
        while len(lünk)<=5: #eeldame, et olulised sõnad on pikemad (siia saaks ka muid kriteeriume panna - näiteks aastaarvude õppimiseks lünk koosneks ainult numbritest
            lünk=tekst_listina[randint(1,len(tekst_listina))-1]
...
def õige():
    ...
    Button(win, text='Lahku mängust', command=partial(lõpeta,raam)).pack() #sulgeb mängu
...
def näita():
    ...
    nupp = ttk.Button(alamraam, text="Lahku mängust", command=partial(lõpeta,asendus))
    nupp.pack(padx=5,side=LEFT)
    alamraam.pack()
